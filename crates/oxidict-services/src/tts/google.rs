...
use oxidict_core::{Language, Result, TtsRequest};
...
impl Tts for Google {
    ...
    async fn tts(&self, req: TtsRequest) -> Result<Vec<u8>> {
        ...
        let bytes = oxidict_net::get_bytes(url).await?;
        Ok(bytes.to_vec())
    }
}
...
fn google_tk(text: &str) -> String {
    ...
    let mut a: i32 = 0; // TKK 第一段（oxidict 固定为 0）
    for &ei in &e {
        a = a.wrapping_add(ei);
        a = xr(a, &[10, 6]);
    }
