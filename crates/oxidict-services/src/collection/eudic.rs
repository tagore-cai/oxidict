...
use oxidict_core::{CollectionRequest, Error, Result};
use serde_json::json;
...
impl Eudic {
    ...
    async fn check_category(&self, name: &str, token: &str) -> Result<i64> {
        ...
        let resp = oxidict_net::get_with_headers(CATEGORY_URL, &headers)
            .query(&[("language", "en")])
            .send()
            .await
            .map_err(net_err)?;
        ...
        // 未找到，创建分类。
        let create = oxidict_net::post_with_headers(CATEGORY_URL, &headers)
            .json(&json!({ "language": "en", "name": name }))
            .send()
            .await
            .map_err(net_err)?;
        let create = oxidict_net::check(create).await?;
    ...
    async fn add_word(&self, category_id: i64, word: &str, token: &str) -> Result<()> {
        ...
        let resp = oxidict_net::post_with_headers(WORDS_URL, &headers)
            .json(&body)
            .send()
            .await
            .map_err(net_err)?;
...
/// reqwest 错误转 `Error::Network`，对齐 `oxidict-net` 的内部约定。
