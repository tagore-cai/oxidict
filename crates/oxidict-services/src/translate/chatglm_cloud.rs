//! ChatGLM（沙拉云中转）翻译。
//!
//! 移植自 `src/services/translate/chatglm_cloud/`。
...
#[async_trait]
impl Translator for ChatglmCloud {
    fn id(&self) -> &str {
        "chatglm_cloud"
    }
    ...
    async fn translate(&self, req: TranslateRequest) -> Result<TranslateResult> {
        ...
        let content = extract_content(&v)
            .ok_or_else(|| Error::Service(format!("chatglm_cloud: 无翻译结果: {}", text)))?;
