...
use oxidict_core::{Error, Language, Result, StreamSink, TranslateRequest, TranslateResult};
...
pub async fn chat_completions(
    ...
    let mut rb = oxidict_net::post_with_headers(url, headers).json(&body);
    if !stream {
    ...
    if stream {
        ...
        let mut stream = oxidict_net::sse_text(resp);
        let mut target = String::new();
        while let Some(line) = stream.next().await {
            let line = line?;
            if let Some(payload) = oxidict_net::sse_payload(&line)
...
pub async fn ollama_chat(url: &str, body: Value) -> Result<TranslateResult> {
    let resp = oxidict_net::post_json_value(url, &body).await?;
    let content = resp
        .get("message")
