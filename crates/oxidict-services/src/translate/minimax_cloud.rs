//! MiniMax（沙拉云中转）翻译。
//!
//! 移植自 `src/services/translate/minimax_cloud/`。
...
const MODELS: &[(&str, &str)] = &[("minimax-m2", "minimax-m2")];
...
#[async_trait]
impl Translator for MinimaxCloud {
    fn id(&self) -> &str {
        "minimax_cloud"
    }
    ...
    async fn translate(&self, req: TranslateRequest) -> Result<TranslateResult> {
        ...
        let content = extract_content(&v)
            .ok_or_else(|| Error::Service(format!("minimax_cloud: 无翻译结果: {}", text)))?;
