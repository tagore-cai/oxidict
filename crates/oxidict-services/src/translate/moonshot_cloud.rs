//! Moonshot（沙拉云中转）翻译。
//!
//! 移植自 `src/services/translate/moonshot_cloud/`。
...
const MODELS: &[(&str, &str)] = &[("kimi-k2", "kimi-k2")];
...
#[async_trait]
impl Translator for MoonshotCloud {
    fn id(&self) -> &str {
        "moonshot_cloud"
    }
    ...
    async fn translate(&self, req: TranslateRequest) -> Result<TranslateResult> {
        ...
        let content = extract_content(&v)
            .ok_or_else(|| Error::Service(format!("moonshot_cloud: 无翻译结果: {}", text)))?;
