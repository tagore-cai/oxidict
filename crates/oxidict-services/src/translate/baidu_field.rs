//! 百度翻译（领域版 / 垂直领域）。
//!
//! 移植自 `src/services/translate/baidu_field/index.jsx`。
//! 与通用版相比仅多了 `field`（领域）参数，签名串为
...
pub struct BaiduField;

const ENDPOINT: &str = "https://fanyi-api.baidu.com/api/trans/vip/fieldtranslate";
...
#[async_trait]
impl Translator for BaiduField {
    fn id(&self) -> &str {
        "baidu_field"
    }
    ...
    async fn translate(&self, req: TranslateRequest) -> Result<TranslateResult> {
        ...
        let field = req.str("field").unwrap_or("it");

        let from = self.map_language(req.from);
        let to = self.map_language(req.to);
        let salt = uuid_v4();
        let sign = md5_hex(&format!("{}{}{}{}{}", appid, req.text, salt, field, secret));

        let url = format!(
            "{}?q={}&from={}&to={}&appid={}&salt={}&sign={}&domain={}",
            ENDPOINT,
