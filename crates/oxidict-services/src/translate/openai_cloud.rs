//! OpenAI（沙拉云中转）翻译。
//!
//! 移植自 `src/services/translate/openai_cloud/`。
//! 走沙拉自己的在线中转（`/api/chat/completions`），鉴权头为 `Authorization: Bearer <apiKey>`，
//! 请求与响应均为 OpenAI 兼容格式，SSE 增量字段为 `choices[0].delta.content`。
...
#[async_trait]
impl Translator for OpenaiCloud {
    fn id(&self) -> &str {
        "openai_cloud"
    }
    ...
    async fn translate(&self, req: TranslateRequest) -> Result<TranslateResult> {
        ...
        let rb = oxidict_net::post_with_headers(url, &headers).json(&body);
        ...
        if let Some(sink) = &req.on_stream {
            ...
            let mut s = oxidict_net::sse_text(resp);
            while let Some(line) = s.next().await {
                let line = line?;
                if let Some(data) = oxidict_net::sse_payload(&line)
        ...
        let content = extract_content(&v)
            .ok_or_else(|| Error::Service(format!("openai_cloud: 无翻译结果: {}", text)))?;
