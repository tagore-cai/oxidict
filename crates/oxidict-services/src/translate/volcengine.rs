...
impl Translator for Volcengine {
    ...
    async fn translate(&self, req: TranslateRequest) -> Result<TranslateResult> {
        ...
        let body_hash = oxidict_net::sha256_hex(body_str.as_bytes());

        let format_date = oxidict_net::aws_now();
        let short_date = oxidict_net::aws_date();
        ...
        let canonical_request =
            format!("POST\n/\n{norm_query}\n{canonical_headers}\n{signed_headers}\n{body_hash}");
        let hashed_canonical = oxidict_net::sha256_hex(canonical_request.as_bytes());
        ...
        let k_date = oxidict_net::hmac_sha256_raw(secret.as_bytes(), short_date.as_bytes());
        let k_region = oxidict_net::hmac_sha256_raw(&k_date, REGION.as_bytes());
        let k_service = oxidict_net::hmac_sha256_raw(&k_region, SERVICE.as_bytes());
        let signing_key = oxidict_net::hmac_sha256_raw(&k_service, b"request");
        let signature = oxidict_net::hmac_sha256_hex(&signing_key, &string_to_sign);
