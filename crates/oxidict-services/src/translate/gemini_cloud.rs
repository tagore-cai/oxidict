//! Gemini（沙拉云中转）翻译。
//!
//! 移植自 `src/services/translate/gemini_cloud/`。
//! 与 openai_cloud 一样走沙拉在线中转的 OpenAI 兼容 `/api/chat/completions`，
...
#[async_trait]
impl Translator for GeminiCloud {
    fn id(&self) -> &str {
        "gemini_cloud"
    }
    ...
    async fn translate(&self, req: TranslateRequest) -> Result<TranslateResult> {
        ...
        let content = extract_content(&v)
            .ok_or_else(|| Error::Service(format!("gemini_cloud: 无翻译结果: {}", text)))?;
