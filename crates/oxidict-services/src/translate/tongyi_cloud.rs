//! 通义（沙拉云中转）翻译。
//!
//! 移植自 `src/services/translate/tongyi_cloud/`。
...
const MODELS: &[(&str, &str)] = &[("qwen3-max", "qwen3-max")];
...
#[async_trait]
impl Translator for TongyiCloud {
    fn id(&self) -> &str {
        "tongyi_cloud"
    }
    ...
    async fn translate(&self, req: TranslateRequest) -> Result<TranslateResult> {
        ...
        let content = extract_content(&v)
            .ok_or_else(|| Error::Service(format!("tongyi_cloud: 无翻译结果: {}", text)))?;
