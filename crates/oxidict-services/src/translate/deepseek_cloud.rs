//! DeepSeek（沙拉云中转）翻译。
//!
//! 移植自 `src/services/translate/deepseek_cloud/`。
...
const MODELS: &[(&str, &str)] = &[("deepseek-v4-flash", "deepseek-v4-flash")];
...
#[async_trait]
impl Translator for DeepseekCloud {
    fn id(&self) -> &str {
        "deepseek_cloud"
    }
    ...
    async fn translate(&self, req: TranslateRequest) -> Result<TranslateResult> {
        ...
        let content = extract_content(&v)
            .ok_or_else(|| Error::Service(format!("deepseek_cloud: 无翻译结果: {}", text)))?;
