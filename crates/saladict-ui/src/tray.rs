...
impl AutoCopyMode {
    ...
    fn from_config() -> Self {
        let v = saladict_core::config::config().get_or(
...
pub fn install(sender: UnboundedSender<TrayCommand>) -> anyhow::Result<()> {
    ...
    let dev_mode =
        saladict_core::config::config().get_or(saladict_core::config::keys::DEV_MODE, false);
    ...
    std::thread::Builder::new()
        ...
        .spawn(|| {
            ...
            for event in receiver.iter() {
                ...
                let action = saladict_core::config::config()
                    .get::<String>(saladict_core::config::keys::TRAY_CLICK_EVENT)
                    .unwrap_or_default();
