...
use oxidict_core::config::{config, keys, parse_instance};
use oxidict_core::i18n::{t, t_args};
use oxidict_core::{Language, TranslateRequest, TranslateResult};
use oxidict_services::{instance_display_name, spawn_translate};
use std::sync::Arc;
...
fn persist_window_bounds(window: &Window) {
    ...
    let store = config();
    let _ = store.set(
        oxidict_core::config::keys::TRANSLATE_WINDOW_WIDTH,
        &b.size.width.as_f32(),
    );
    let _ = store.set(
        oxidict_core::config::keys::TRANSLATE_WINDOW_HEIGHT,
        &b.size.height.as_f32(),
    );
    let _ = store.set(
        oxidict_core::config::keys::TRANSLATE_WINDOW_POSITION_X,
        &b.origin.x.as_f32(),
    );
    let _ = store.set(
        oxidict_core::config::keys::TRANSLATE_WINDOW_POSITION_Y,
        &b.origin.y.as_f32(),
    );
}
...
impl TranslateWindow {
    pub fn new(window: &mut Window, cx: &mut Context<Self>) -> Self {
        ...
        let instances = store.service_list(oxidict_core::config::keys::TRANSLATE_SERVICE_LIST);
        let cards = instances
            .iter()
            .map(|instance| Card {
        ...
        if store.get_or(oxidict_core::config::keys::DYNAMIC_TRANSLATE, false) {
        ...
        let remember_size = store.get_or(
            oxidict_core::config::keys::TRANSLATE_REMEMBER_WINDOW_SIZE,
            false,
        );
        let close_on_blur = store.get_or(oxidict_core::config::keys::TRANSLATE_CLOSE_ON_BLUR, true);
    ...
    fn run_translate(&mut self, text: String, window: &mut Window, cx: &mut Context<Self>) {
        ...
        if from == Language::Auto
            ...
            && let Some(detected) = oxidict_platform::detect::detect(&text)
        {
            from = detected;
        }
