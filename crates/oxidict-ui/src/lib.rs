...
fn mouse_follow_origin(width: f32, height: f32) -> Point<Pixels> {
    use oxidict_platform::mouse::{cursor_position, monitor_at_cursor};
        ...
        while let Some(cmd) = rx.recv().await {
            match cmd {
                TrayCommand::SelectionTranslate => {
                    ...
                    let text = match oxidict_core::runtime::handle()
                        .spawn_blocking(oxidict_platform::selection::selected_text)
                        .await
                    {
                        Ok(Ok(text)) => text,
                        Ok(Err(e)) => {
                ...
                TrayCommand::OcrRecognize | TrayCommand::OcrTranslate => {
                    match oxidict_core::runtime::handle()
                        .spawn_blocking(oxidict_platform::screenshot::capture_interactive)
                        .await
                    {
                ...
                TrayCommand::SetAutoCopy(mode) => {
                    let _ = oxidict_core::config::config().set(
                ...
                TrayCommand::ViewLog => {
                    let dir = oxidict_core::config::config().app_dir().join("logs");
                    if let Err(e) = oxidict_platform::shell::open_path(&dir) {
                        log::warn!("打开日志目录失败: {e}");
                    }
                }
                TrayCommand::Restart => {
                    ...
                    let _ = std::fs::remove_file(oxidict_core::config::runtime_lock_path());
