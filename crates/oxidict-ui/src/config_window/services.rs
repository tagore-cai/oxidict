...
use oxidict_core::config::{config, parse_instance};
use oxidict_core::i18n::{t, t_args};
use oxidict_core::schema::ConfigField;
use oxidict_services::services;
...
impl ConfigWindow {
    ...
    fn append_instance(&mut self, kind: ServiceKind, base_id: &str, display: String) {
        ...
        let key = if list.iter().any(|k| parse_instance(k).0 == base_id) {
            let suffix: String = oxidict_net::uuid_v4().chars().take(8).collect();
            format!("{}@{}", base_id, suffix)
        } else {
            base_id.to_string()
        };

        let mut cfg = oxidict_core::ServiceConfig::new();
        cfg.insert(
            oxidict_services::INSTANCE_NAME_KEY.to_string(),
...
impl ConfigWindow {
    pub(super) fn render_service_page(&mut self, cx: &mut Context<Self>) -> gpui_kit::AnyElement {
        ...
        for (list_ix, key) in list.iter().enumerate() {
            let key = key.clone();
            let name = oxidict_services::instance_display_name(&key, &store);
