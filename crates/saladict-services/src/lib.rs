...
impl Services {
    ...
    pub fn resolve_translator(
        ...
        let name = saladict_core::config::parse_instance(instance).0;
        let svc = self
            .translator(name)
...
) -> saladict_core::Result<String> {
    let store = saladict_core::config::config();
    let (svc, cfg) = services().resolve_recognizer(instance, &store)?;
    svc.recognize(saladict_core::RecognizeRequest { config: cfg, ..req })
        .await
}
...
) -> saladict_core::Result<Vec<u8>> {
    let store = saladict_core::config::config();
    let (svc, cfg) = services().resolve_tts(instance, &store)?;
    svc.tts(saladict_core::TtsRequest { config: cfg, ..req })
        .await
}
...
pub async fn translate_first_enabled(
    store: &ConfigStore,
    req: saladict_core::TranslateRequest,
) -> saladict_core::Result<(String, saladict_core::TranslateResult)> {
...
pub fn translate_first_enabled_blocking(
    ...
    let store = saladict_core::config::config();
    saladict_core::runtime::handle().block_on(translate_first_enabled(&store, req))
}
...
pub fn spawn_recognize(
    ...
    let instance = instance.to_string();
    saladict_core::runtime::spawn(async move { recognize_instance(&instance, req).await })
}
...
pub fn spawn_tts(
    ...
    let instance = instance.to_string();
    saladict_core::runtime::spawn(async move { tts_instance(&instance, req).await })
}
