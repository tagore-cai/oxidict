...
pub fn load(plugin_id: &str, dir: &Path) -> Result<()> {
    ...
    match saladict_core::runtime::handle().block_on(reply_rx) {
        Ok(inner) => inner,
...
async fn load_plugin(
    ...
    // 原生模块 `saladict`。不挂 FileResolver——插件的 main.js 按脚本 eval，
    // 与老宿主一致，不支持模块相对导入。
    rt.set_loader(
        rquickjs::loader::BuiltinResolver::default().with_module("saladict"),
        rquickjs::loader::ModuleLoader::default().with_module("saladict", host::SaladictModule),
    )
    .await;
