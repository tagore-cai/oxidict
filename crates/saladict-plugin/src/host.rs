//! 宿主能力面：`saladict` 原生模块 + 全局对象 + legacy `utils`。
//!
//! 三种注入方式并存，插件按习惯任选：
//! 1. ESM：`import { http_request, utils } from "saladict"`（原生模块）；
//! 2. 全局对象：`globalThis.saladict`；
//! 3. legacy `utils`（挂在入口函数的 options 参数里传入）。
//!
//! HTTP 走 reqwest（saladict-net），插件脚本自身不需要网络 capability；
...
/// 原生模块 `saladict`，注册进每个插件 VM 的 loader。
pub struct SaladictModule;

impl ModuleDef for SaladictModule {
...
pub fn inject_globals(ctx: &Ctx<'_>) -> JsResult<()> {
    ...
    let saladict = Object::new(ctx.clone())?;
    saladict.set(
        "http_request",
        Function::new(ctx.clone(), Async(http_request))?,
    )?;
    ...
    saladict.set("run", Function::new(ctx.clone(), run)?)?;
    saladict.set("utils", build_utils(ctx)?)?;
    globals.set("saladict", saladict)?;
...
/// legacy `utils.http` / `saladict.http_request` 的宿主代发实现。
...
async fn http_request(
    ...
    let resp = saladict_net::check(resp)
        .await
