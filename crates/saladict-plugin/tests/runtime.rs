...
use saladict_plugin::runtime::{self, CallRequest};
...
function translate(text, from, to, options) {
  ...
  if (text === "global") {
    return typeof saladict === "object" ? "has-saladict" : "no-saladict";
  }
  return text + "|" + from + "|" + to;
}
"#;
...
fn plugin_runtime_end_to_end() {
    let root = std::env::temp_dir().join(format!("saladict-plugin-test-{}", std::process::id()));
    ...
    saladict_core::config::set_global(store);
    ...
    let call = |input: &str, config: serde_json::Map<String, serde_json::Value>| {
        saladict_core::runtime::handle().block_on(runtime::call(CallRequest {
    ...
    assert_eq!(out, serde_json::json!("has-saladict"));
    ...
    // 8. 未加载的插件报清晰错误
    let err = saladict_core::runtime::handle()
