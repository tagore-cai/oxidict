//! oxidict-plugin：`.potext` 插件运行时，基于 rquickjs（QuickJS）。
//!
...
/// 让插件错误能沿 `oxidict_core::Error` 一路向上传，
/// 服务 trait 的错误类型保持唯一。
impl From<Error> for oxidict_core::Error {
    fn from(e: Error) -> Self {
        match e {
            Error::Plugin(m) => oxidict_core::Error::Plugin(m),
            Error::Io(e) => oxidict_core::Error::Io(e),
            Error::Json(e) => oxidict_core::Error::Json(e),
        }
    }
}
...
pub fn load_installed(store: &oxidict_core::config::ConfigStore) -> Vec<String> {
    use oxidict_core::ServiceKind;
        ...
        for plugin_id in install::installed(kind, store) {
            ...
            match runtime::load(&plugin_id, &dir) {
                Ok(()) => {
                    ...
                    match kind {
                        ServiceKind::Translate => oxidict_services::services()
                            .register_translator(std::sync::Arc::new(service)),
                        ServiceKind::Recognize => oxidict_services::services()
                            .register_recognizer(std::sync::Arc::new(service)),
                        ServiceKind::Tts => {
                            oxidict_services::services().register_tts(std::sync::Arc::new(service))
                        }
                        ServiceKind::Collection => oxidict_services::services()
